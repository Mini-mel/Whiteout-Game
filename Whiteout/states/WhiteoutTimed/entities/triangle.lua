...
function ent:load( x, y )
	ccheck = 0
	self:setPos( x, y)
	self.image = love.graphics.newImage("textures/triangle-128.png")
...
function ent:update(dt)
	...
	--so it can manipulate a fixed y
	self.y = self.starty + math.sin(love.timer.getTime() - self.start) * 300
	self.x = self.x + 300* dt
	if self.x >= (800 + 200) then
